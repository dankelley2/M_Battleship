...
namespace Battleship
{
    public partial class MainWindow : Form
    {
        ...
        {
                remote.SendData(Player.GetAllPlayerIPsExcept(MyIPAddress), MyIPAddress + ";" + target + ";" + HitMiss, "SR");
        }
        ...
        {
            remote.SendData(Player.GetAllPlayerIPsExcept(MyIPAddress), CPlayer.address , "CT");
        }
        ...
        private void Get_Data()
        {
            ...
            if (message != null)
            {
                ...
                else if (message.Type == "SR") //Shot Response
                {
                    ...
                    remote.SendData(Player.GetAllPlayerIPsExcept(MyIPAddress), "Shot fired on " + P.Name + "!", "M");
                    if (Convert.ToBoolean(shot[2]) == true)
                    {
                        remote.SendData(Player.GetAllPlayerIPsExcept(MyIPAddress), "It's a Hit","M");
                    ...
                    {
                        remote.SendData(Player.GetAllPlayerIPsExcept(MyIPAddress), "It was a doozy", "M");
        ...
        {
            ColorQueue.Enqueue(Color.FromArgb(150, Color.Aqua));
            ...
            Player.AddPlayer(MyIPAddress, HostName);
            ...
            if (IsHost)
            {
                if (Player.GetAllPlayerIPsExcept(MyIPAddress).Count > 0)
                {
                    if (Player.IsEveryoneReadyToPlay())
                    {
                        ...
                        gamePart += 1;
                        remote.SendData(Player.GetAllPlayerIPsExcept(MyIPAddress), "Players are Ready. Game starting.", "M");
                        ...
                        remote.SendData(Player.GetAllPlayerIPsExcept(MyIPAddress), PlayOrderMessage, "M");
                    }
                    else
                    {
                        AskReady();
        ...
        private void CreateEnemyGrids()
        {
            ...
            foreach(Player P in Player.Playerlist)
            {
                ...
                Gameboard newBoard = new Gameboard(P.address, GetNewBoardColor());
